...
ENERGY_KW = ["crude", "brent", "wti", "opec", "lng", "petroleum", "oil price", "oil demand",
             ...
             # 에너지 수급계획·전망(가스공사 장기 천연가스 수급계획 등 에너지 계획·전망 문서)
...
REPORT_HINTS = [
    ...
    "가스공사", "kogas", "석유공사", "knoc",   # 수급계획·시장분석 등 발간(단, 아래 사내 행정·행사 뉴스는 제목필터로 제외). 오피넷=순수 시세라 제외
...
QUICK_LINKS = {
    ...
    "🇮🇷 이란·역내 매체": [("Tehran Times", "https://www.tehrantimes.com/"),
        ...
        ("Mehr News", "https://en.mehrnews.com/"),
        ("Fars News", "https://www.farsnews.ir/en"),
        ("ISNA (English)", "https://en.isna.ir/"),
        ("Nour News", "https://nournews.ir/En"),
    ...
    "📑 국내 연구기관": [("대외경제정책연구원(KIEP)", "https://www.kiep.go.kr/"),
        ...
        ("삼정KPMG 경제연구원(ERI)", "https://kpmg.com/kr/ko/insights/eri.html"),
        ("삼일PwC(Insights)", "https://www.pwc.com/kr/ko/insights.html"),
        ("딜로이트 안진(Insights)", "https://www2.deloitte.com/kr/ko/insights.html"),
        ("EY한영", "https://www.ey.com/ko_kr"),
        ("맥킨지 코리아", "https://www.mckinsey.com/kr/our-insights/mckinsey-insights"),
        ("BCG 코리아", "https://www.bcg.com/korea"),
    ...
    "🌐 해외 연구기관": [("IEA 국제에너지기구", "https://www.iea.org/"),
        ...
        ("Baker Institute(에너지·카타르 LNG)", "https://www.bakerinstitute.org/"),
        ("Oxford Institute for Energy Studies", "https://www.oxfordenergy.org/"),
        ("GECF 가스수출국포럼", "https://www.gecf.org/"),
        ("IEEFA 에너지경제·재무분석연구소", "https://ieefa.org/"),
        ("Wood Mackenzie", "https://www.woodmac.com/"),
        ("Rystad Energy", "https://www.rystadenergy.com/"),
        ("AGSIW 아랍걸프국연구소", "https://agsiw.org/"),
        ("The Washington Institute", "https://www.washingtoninstitute.org/"),
        ("ECFR 유럽외교협회", "https://ecfr.eu/"),
        ("Control Risks(지정학·공급망 리스크)", "https://www.controlrisks.com/"),
        ("Moody's", "https://www.moodys.com/"),
        ("Fitch Ratings", "https://www.fitchratings.com/"),
        ("McKinsey & Company", "https://www.mckinsey.com/"),
        ("BCG 보스턴컨설팅그룹", "https://www.bcg.com/"),
...
LANG = {
 "ko": {
  ...
  "search_count": "건 표시",
  "sum_head": "🧭 주요 이슈별 요약", "sum_head_today": "🧭 주요 이슈별 요약", "sum_head_flat": "🧭 주요 핵심 요약", "sum_head_wk": "🧭 지난주 언론 동향", "ai": "AI 자동요약",
  ...
  "foot": ("본 페이지는 카타르·이란·기타 주요 국가 및 한국의 공개 언론 보도와 유관기관 자료를 자동으로 수집·요약합니다. "
           ...
           "심층 보고서는 국내외 연구기관·국제기구·컨설팅펌의 발간물 게시판·RSS를 직접 수집(발행처 도메인 대상 뉴스 검색으로 보강)하며, "
           "중동 정세·이란 전쟁과 그 산업·공급망·에너지 여파에 관한 분석·보고서만 선별합니다. 보고서 목록은 개전(2026-02-28) 이후 발행분을 국내·해외로 나눠 발행일 최신순으로 누적합니다. "
           ...
           "또한 정세 관련 카타르 정부 동향 섹션은 카타르 국왕(Emir)·총리·외교부·국방부·내무부·정부커뮤니케이션실(GCO)·카타르에너지·국영통신(QNA) 등 카타르 정부·국영기관의 전쟁·정세 관련 공식 성명·발표·조치만 별도로 선별해 정리한 항목입니다. "
 ...
 "en": {
  ...
  "foot": ("This page automatically collects and summarizes public news coverage and material from relevant institutions "
           ...
           "are collected directly from the publication boards and RSS of domestic and international research institutes, international organizations and consultancies "
           "(supplemented by news search targeting those publishers), keeping only analyses and reports on the Middle East situation, the Iran war and their industrial, supply-chain and energy repercussions. "
           "The report list accumulates items published since the war's outbreak (2026-02-28), split into domestic and overseas and ordered by publication date (newest first). "
           "Collection and summarization rely on each outlet's publicly available headline and snippet; "
 ...
 "ar": {
  ...
  "bg_tag": "خلفية", "bg_head": "خلفية الحرب ومسارها (بالكورية)", "bg_link": "الخلفية", "tl_link": "الجدول الزمني (2026)",
  ...
  "foot": ("تجمّع هذه الصفحة وتلخّص تلقائيًا التغطيات الإعلامية العامة ومواد الجهات المعنية في قطر وإيران وسائر الدول الرئيسية وكوريا. "
           ...
           "بكلمات مفتاحية مثل قطر والشرق الأوسط والطاقة؛ وتُجمَع التقارير المعمّقة مباشرةً من لوحات إصدارات وخلاصات RSS لمراكز الأبحاث المحلية والدولية والمنظمات الدولية وشركات الاستشارات "
           "(مع تعزيزها ببحث إخباري يستهدف تلك الجهات)، مع الإبقاء فقط على التحليلات والتقارير المتعلقة بأوضاع الشرق الأوسط وحرب إيران وتداعياتها الصناعية وسلاسل التوريد والطاقة. "
           "وتتراكم قائمة التقارير منذ اندلاع الحرب (2026-02-28)، مقسّمةً إلى محلية ودولية ومرتّبةً بحسب تاريخ النشر (الأحدث أولًا). "
           "ويعتمد الجمع والتلخيص على العنوان والمقتطف المتاحَين علنًا لكل وسيلة؛ "
...
REPORT_DOMAINS = [
    ...
    "asaninst.org", "sejong.org", "kita.net", "kogas.or.kr", "knoc.or.kr",
...
_NON_REPORT_TITLE_SUBSTR = (
    ...
    # 공기업 사내 행정·인사·행사 뉴스(가스공사·석유공사 등) — 분석 보고서가 아님
    ...
    for s in range(0, len(items), CHUNK):
        ...
        try:
            ...
            prompt = (
                ...
                "**특히 카타르 정부·지도부·국영기관(국왕(Emir)·총리·외교부·국방부·내무부·정부커뮤니케이션실(GCO)·"
                "카타르에너지·국영통신(QNA) 등)의 발언·조치·동향 기사는 정세 관련성이 다소 애매해도 반드시 '유지'하세요 "
                "— 카타르 정부 동향 섹션이 이 풀에서 만들어지므로 여기서 빠지면 정부 동향이 누락됩니다.**\n"
...
def gemini_issues(pool, win_label, weekly=False):
    ...
    prompt = (
        ...
        "【단일 매체로 단정하기 전에 반드시 복수 매체 확인】 어떤 주장을 요약할 때, 그 주장이 정말 한 매체에만 있는지 [기사 목록] 전체(국내 전재·현지·국제 매체 모두)를 먼저 훑어보고 판단하세요. "
        "둘 이상의 매체가 같은 주장을 전했다면 특정 한 매체명을 괄호로 콕 집어 '출처'처럼 요약에 넣지 말고, 주체(예: 'IRGC가 … 주장함')로만 서술하세요"
        ...
        "단, 그 피해·위협 주장이 **단일·비정통 매체(예: 지역 밖 2차 매체) 단독 보도이고 1차·현지 정통 소스가 없으면** 본문 끝에 "
        "**그 보도한 단일 매체명을 밝혀** '(○○ 단독 보도, 교차확인 필요)' 형태로 붙일 것(○○에 실제 매체명을 넣으세요. 예: '(Jerusalem Post 단독 보도, 교차확인 필요)'. "
        "매체명은 **한국어로 음차하지 말고 원문 표기(영문 등) 그대로** 쓸 것(예: '제루살렘포스트' X → 'Jerusalem Post' O, 'Al Jazeera', 'Reuters' 등도 원문 그대로). "
        "가능하면 원·1차 매체명을, 매체명을 도저히 특정할 수 없을 때만 '(단일 매체 보도, 교차확인 필요)'로 표기)), "
...
def _gov_ai_from_hits(hits):
    ...
    prompt = (
        ...
        "'긴장 상황을 논의함'처럼 뭉뚱그리지 말고 기사에 보도된 구체적 논의·촉구·합의 내용(무엇을 다뤘는지)을 담으세요. "
        "'중동'으로 표기(원문 'West Asia/서아시아'도 '중동'으로), '서아시아(중동)' 병기 금지. 중동+글로벌 안정을 함께 다뤘으면 실제 범위를 그대로 반영. "
        + _SELFCONTAINED_RULE +
        ...
        "카타르 국영통신(QNA) 등 카타르 매체가 단순 보도한 것)는 카타르 정부 동향이 아니므로 **반드시 제외**하세요. "
        "이런 기사만 있고 카타르 정부·지도부 관련 동향이 전혀 없으면 주저 없이 {\"gov\":[]}를 반환하세요. "
...
def _gov_kw_fallback(qpool):
    ...
    키워드가 뚜렷한 기사가 있으면, 그 기사들만 따로 2차 AI 요약해 '동향 내용 자체'를 채우는 안전장치.
    2차 AI마저 (근거 부족·카타르 정부 무관으로) 빈 배열을 주면 억지로 채우지 않고 빈 값을 반환한다
    → 섹션은 '특이사항 없음'으로 정직하게 표시.
    (원문 헤드라인 나열은 폐기 — 키워드 오탐 기사·영어 원문이 카타르 정부 동향인 양 노출되던 문제 차단.
     과거의 '…확인 요망(자동 감지)' 식 안내문도 폐기.)"""
    hits, seen = [], set()
    for x in qpool:
        t = x.get("title") or ""
        low = t.lower()
    ...
    if not hits:
        return []
    # 검출 기사만 넘겨 구체적 동향을 2차 AI 요약. 근거가 없으면 []( = '특이사항 없음')로 정직하게 둔다.
...
def _qatar_gov_pool(items):
    ...
    단, 보고서(연구기관 분석)는 '정부 행위'가 아니므로 정부 동향 풀에서 제외한다."""
    qat = [x for x in items if (x.get("qatar") or x.get("region") == "qatar") and not x.get("report")]
...
def gemini_qatar_gov(pool, win_label):
    ...
    prompt = (
        ...
        "**【기사에 명시된 카타르 정부 행위만 — 추론·해석 금지(매우 중요)】 각 불릿은 기사에 '명시적으로 보도된 카타르 정부·지도부의 구체적 행위(성명 발표·통화·회담·중재·서명·규탄·촉구·환영 등, 누가·무엇을·언제)'만 담으세요. "
        "기사 주제가 타국·타 주체의 행위(예: 이스라엘의 거부·미국의 발표·이란의 조치)이고 카타르는 배경·맥락으로만 언급될 뿐 그 기사에 카타르 정부의 구체적 행위가 없으면 **불릿으로 만들지 마세요**(카타르의 관여·역할을 추론해 만들어내지 말 것). "
        "**카타르의 역할·입지에 대한 일반적 서술('중재국 역할을 지속해옴'), 향후 영향·전망 추측('~에 영향을 줄 수 있는 사안임', '~할 가능성', '~로 해석됨'), 배경 해설은 정부 '동향'이 아니므로 금지** — 오직 이번 기간에 실제로 일어난, 기사에 적힌 카타르 정부의 행위만.** "
        ...
        "**【과거 사건 재탕·회고 기사 제외(중요)】 이 섹션은 '이번 모니터링 기간에 새로 일어난' 카타르 정부의 행위만 담습니다. 기사의 게시 시각이 최신이어도, 그 내용이 **수개월 전 발생한 과거 사건**(예: 개전 초기의 에너지 시설 피해·불가항력 선언, 그때 발표된 생산능력 손실 전망 등)을 뒤늦게 재보도·요약·회고하거나 그 여파를 해설하는 것이면 '새 동향'이 아니므로 **제외**하세요. '이번 기간에 카타르 정부가 무엇을 했는가'가 아니라 '과거에 무슨 일이 있었는가'를 설명하는 기사는 넣지 마세요. "
        "**【에너지 시황·경영 코멘트 제외】 카타르에너지(QatarEnergy) 관련은 '전쟁·정세에 따른 공식 조치·성명'(예: 불가항력 선언, 공급 중단·재개 발표, 정세 관련 공식 입장)만 포함하세요. 단순 생산량·수출량·가격 전망, 투자·증산 계획, 시장 영향 분석 같은 **에너지 시황·기업 경영성 코멘트는 '정세 관련 정부 동향'이 아니므로 제외**하세요. "
        ...
        "**【빈약한 요약 금지 — 실제 내용을 담을 것】 '긴장 상황을 논의함/협의함'처럼 알맹이 없이 뭉뚱그리지 말고, 기사에 보도된 **구체적 논의·촉구·합의 내용**(예: 가자 정전·인질, 호르무즈 통항·해상안전, 에너지·글로벌 시장 안정, 확전 저지, 특정국에 대한 촉구 등 실제로 무엇을 다뤘는지)을 담아 요약만으로 사안이 이해되게 쓰세요. 통화·회담이면 '무슨 주제로, 어떤 핵심 메시지·입장을, 어떤 맥락에서' 나눴는지를 기사 근거로 충실히 서술하세요(추정·창작 없이, 기사에 있는 구체 내용을 최대한 반영).** "
        "**【용어·범위】 한국어 독자 기준으로 '중동'으로 표기하세요 — 원문이 'West Asia/서아시아'로 써도 '중동'으로 옮기고 '서아시아(중동)' 같은 병기는 쓰지 마세요. 또 논의가 중동에 국한되지 않고 역내·글로벌 안정을 함께 다뤘으면 '중동 및 글로벌 안정'처럼 실제 범위를 그대로 반영하고, 넓은 대화를 '중동 긴장'으로 좁히지 마세요.** "
        + _SELFCONTAINED_RULE +
...
TIMELINE = [
  ...
  {"d": "2026-03-21", "q": False,
   ...
   "ar": "إيران تحاول ضرب القاعدة الأمريكية–البريطانية في دييغو غارسيا بالمحيط الهندي بصاروخ باليستي بعيد المدى — اتساع رقعة الحرب."},
  {"d": "2026-03-22", "q": True,
   "ko": "카타르 군용헬기 추락으로 7명 사망(카타르군 4명·튀르키예 국적 3명).",
   "en": "A Qatari military helicopter crash kills 7 (4 Qatari soldiers and 3 Turkish nationals).",
   "ar": "تحطّم مروحية عسكرية قطرية يودي بحياة 7 أشخاص (4 جنود قطريين و3 مواطنين أتراك)."},
  {"d": "2026-03-25", "q": False,
  ...
  {"d": "2026-06-06", "q": False,
   ...
   "ar": "تعثّر محادثات واشنطن–طهران بسبب 24 مليار دولار من الأصول الإيرانية المجمّدة (رضائي)."},
  {"d": "2026-06-07", "q": False,
   "ko": "이란, 이스라엘 북부에 탄도미사일 발사 — 이스라엘은 이란 군사시설 보복 타격.",
   "en": "Iran fires ballistic missiles at northern Israel; Israel retaliates against Iranian military sites.",
   "ar": "إيران تطلق صواريخ باليستية على شمال إسرائيل؛ وإسرائيل تردّ بضرب مواقع عسكرية إيرانية."},
  {"d": "2026-06-11", "q": False,
   "ko": "이란, 호르무즈 해협 폐쇄 발표(1차) — 통항·유가 불안 재점화.",
   "en": "Iran announces closure of the Strait of Hormuz (first time) — reigniting shipping and oil-price turmoil.",
   "ar": "إيران تعلن إغلاق مضيق هرمز (للمرة الأولى) — ما يعيد إشعال اضطراب الملاحة وأسعار النفط."},
  {"d": "2026-06-18", "q": False,
   "ko": "미국, 이란 항구에 대한 해군 봉쇄 해제 — 6/17 잠정합의 이행의 일환.",
   "en": "The US lifts its naval blockade of Iranian ports as part of implementing the June 17 interim deal.",
   "ar": "الولايات المتحدة ترفع الحصار البحري عن الموانئ الإيرانية ضمن تنفيذ اتفاق 17 يونيو المؤقت."},
  {"d": "2026-06-28", "q": False,
   "ko": "트럼프, '합의 위반 시 이란은 더 이상 존재하지 않을 것' 경고 — 양측 공격 중단·상선 통항 허용에 합의.",
   "en": "Trump warns Iran 'would no longer exist' if the deal is violated — both sides agree to halt attacks and allow vessel passage.",
   "ar": "ترامب يحذّر من أن إيران 'لن يبقى لها وجود' حال خرق الاتفاق — والطرفان يتفقان على وقف الهجمات والسماح بمرور السفن."},
  {"d": "2026-07-12", "q": True,
   "ko": "카타르 아버지 아미르(전 국왕) 셰이크 하마드 빈 칼리파 알사니 서거(향년 74) — 1995~2013년 재위, 전쟁 국면 속 국가적 애도(전쟁과 무관한 자연 서거).",
   "en": "Qatar's Father Amir (former ruler) Sheikh Hamad bin Khalifa Al Thani dies at 74 (ruled 1995–2013) — national mourning amid the war (a natural death unrelated to the conflict).",
   "ar": "وفاة الأمير الوالد (الحاكم السابق) الشيخ حمد بن خليفة آل ثاني عن 74 عاماً (حكَم 1995–2013) — حداد وطني وسط الحرب (وفاة طبيعية لا صلة لها بالنزاع)."},
  {"d": "2026-07-18", "q": False,
...
def gemini_timeline_candidates(pool, win_label):
    ...
    prompt = (
        ...
        "포함: 개전·주요 피격(카타르 본토·에너지시설·미군기지 등)·정전/휴전/합의·확전·대형 에너지 차질(LNG·유가)·중대 인명피해·해협 봉쇄/통항 변화·주요 외교 합의, "
        "그리고 **주체별 피해·손실 현황**(카타르·미군·이란·이스라엘·기타국의 사상자·시설·함정·항공기·에너지 차질 등)과 **GCC 국가들의 주요 발표·공동성명**(특정 중대사건 직후 나오는 것) 등 판세를 바꾼/중대 진전 사건. "
...
var srels=hyper.length?('<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">'+hyper.map(function(h,k){return '<Relationship Id="rId'+(k+1)+'" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink" Target="'+X(h.url)+'" TargetMode="External"/>';}).join('')+'</Relationships>'):'';
var sheet='<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheetPr><pageSetUpPr fitToPage="1"/></sheetPr><dimension ref="A1:'+lc+lr+'"/><sheetViews><sheetView workbookViewId="0"><pane ySplit="2" topLeftCell="A3" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews><sheetFormatPr defaultRowHeight="15"/>'+cols+'<sheetData>'+sd+'</sheetData><autoFilter ref="A2:'+lc+lr+'"/><mergeCells count="1"><mergeCell ref="A1:'+lc+'1"/></mergeCells>'+hl+'<pageMargins left="0.5" right="0.5" top="0.6" bottom="0.6" header="0.3" footer="0.3"/><pageSetup paperSize="9" orientation="portrait" fitToWidth="1" fitToHeight="0"/></worksheet>';
...
var files=[{name:'[Content_Types].xml',data:SB(ct)},{name:'_rels/.rels',data:SB(rels)},{name:'xl/workbook.xml',data:SB(wb)},{name:'xl/_rels/workbook.xml.rels',data:SB(wbr)},{name:'xl/styles.xml',data:SB(styles)},{name:'xl/worksheets/sheet1.xml',data:SB(sheet)}];
if(srels){files.push({name:'xl/worksheets/_rels/sheet1.xml.rels',data:SB(srels)});}
return zip(files);}
...
function exportDocx(){
  ...
  dl(zip([{name:'[Content_Types].xml',data:SB(ct)},{name:'_rels/.rels',data:SB(rels)},{name:'word/document.xml',data:SB(doc)}]),'application/vnd.openxmlformats-officedocument.wordprocessingml.document',fnbase(D)+'.docx');
}
...
<style>
  ...
  *{{box-sizing:border-box}} body{{margin:0;background:var(--bg);color:var(--txt);line-height:1.5;
  ...
  @media (max-width:520px){{h1{{font-size:16.5px}} .qchips a{{padding:6px 11px}}
    ...
    .langrow{{justify-content:space-between;flex-wrap:nowrap;gap:8px}} .lang{{flex:0 1 auto;min-width:0;gap:6px}} .langbtn{{min-width:0;text-align:center;padding:10px 8px;font-size:14px}} .sitelink{{flex:0 0 auto}}
...
<title>{title}</title>
<style>
  :root{{--bg:#0b1220;--panel:#131c2e;--panel2:#0f1729;--line:#243149;--txt:#e6edf7;--muted:#93a1b8;--accent:#6b7280;--gold:#f2b134;--qmaroon:#8A1538}}
  @media (prefers-color-scheme:light){{:root{{--bg:#f4f6fb;--panel:#fff;--panel2:#eef2f9;--line:#dbe2ee;--txt:#14213a;--muted:#5a6b85;--accent:#4b5563}}}}
  *{{box-sizing:border-box}}
  ...
  .tlback{{display:inline-block;font-size:12.5px;font-weight:700;color:#4da3ff;text-decoration:none;margin-bottom:8px}}
  ...
  .tlpage-tag{{font-size:12px;font-weight:800;color:#fff;background:#64748b;border-radius:7px;padding:2px 9px;white-space:nowrap}}
  ...
  .tlfbtn.on{{background:#cbd5e1;color:#334155}}
  ...
  /* A4 인쇄 최적화 */
  @media print{{
    :root{{--bg:#fff;--panel:#fff;--panel2:#f5f5f5;--line:#c9c9c9;--txt:#111;--muted:#555;--accent:#4b5563;--gold:#a9791a}}
...
def main():
    ...
    for lang in LANGS:
        ...
        if is_weekly:
            ...
            # 주간 모니터링 기간: 시작(지난 일요일 07:00) ~ 종료(당일 일요일 07:00) 전체 일시 표기(연도는 시작에만)
            _ws = now_q - timedelta(days=WEEKLY_LOOKBACK_DAYS)
            _wds = L["wdays"]
            wwin = (f"{_ws.year}. {_ws.month}. {_ws.day}.({_wds[_ws.weekday()]}) 07:00"
                    f" ~ {now_q.month}. {now_q.day}.({_wds[now_q.weekday()]}) 07:00"
                    f" ({L['tz']})")
